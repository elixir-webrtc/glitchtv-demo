...
RUN apt-get update -y && apt-get install -y build-essential wget git pkg-config libssl-dev \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/glitchtv ./
